...
端侧需要将这两个字段透传到 SDK 相关接口出参，并在助理详情页、切换助理页、选择助理页的 tag 标签中展示。同时助理 Tab 页导航栏需要通过新增 SDK 接口 `getWeAgentTabInfo` 获取当前助理的 `tagName`、`tagNameEn`，由设置标题的调用方根据客户端语言环境选择实际导航标题。
...
4. 客户端冷启动时，先调用 `getWeAgentUri` 获取 `switchAssistantUri`，再调用新增 SDK 接口 `getWeAgentTabInfo` 获取 `tagName`、`tagNameEn`。
5. `getWeAgentTabInfo` 负责对 `tagName`、`tagNameEn` 做兜底，保证两个字段都有值；助理 Tab 页导航栏在设置标题时只根据客户端语言环境选择 `tagName` 或 `tagNameEn`，并使用 `switchAssistantUri` 作为点击标题时打开切换助理页面的地址。
6. 切换助理页面点击其他助理时，先调用 `getWeAgentDetails` 获取并设置当前助理详情缓存，再根据 `小程序JSAPI接口文档.md` 调用 `openWeAgentCUI`；SDK 收到 `openWeAgentCUI` 调用请求后调用 `getWeAgentTabInfo` 获取最新 `tagName`、`tagNameEn`，由设置标题的逻辑选择导航栏标题，并刷新点击标题时使用的切换助理页面地址。
7. IM 页面点击助理 item 项时，调用 `SkillClientSdkInterfaceV2.md` 中的 `openWeAgent` 打开助理 Tab 页；SDK 在 `openWeAgent` 内完成目标助理详情获取、当前详情缓存写入、URI 组装和 `openWeAgentCUI` 调用后，收到 `openWeAgentCUI` 调用请求时调用 `getWeAgentTabInfo` 获取最新 `tagName`、`tagNameEn`，由设置标题的逻辑选择导航栏标题，并刷新点击标题时使用的切换助理页面地址。
...
4. 不在端侧计算业务助理标签名称；助理 Tab 标题兜底由 `getWeAgentTabInfo` 完成，端侧设置标题时只做语言字段选择。
...
核心方案是：服务端在详情和列表接口统一下发 `tagName`、`tagNameEn`；SDK 在详情、列表、缓存结构中完整透传；新增 `getWeAgentTabInfo` 从当前助理详情缓存读取 `tagName`、`tagNameEn`，并在接口内部完成兜底，保证出参两个字段都有值。助理 Tab 页导航栏在设置标题时再判断客户端语言环境：中文使用 `tagName`，英文使用 `tagNameEn`。
...
sequenceDiagram
    ...
    SDK-->>Host: 返回最新 { tagName, tagNameEn }
...
5. SDK 新增 `getWeAgentTabInfo` 接口，返回 `{ tagName, tagNameEn }`。
6. 助理详情页、切换助理页、选择助理页展示 tag 标签时优先使用 `tagName`、`tagNameEn`，缺失时走现有展示逻辑。
7. 助理 Tab 页冷启动时通过 `getWeAgentUri` 获取切换助理页面地址，通过 `getWeAgentTabInfo` 获取 `tagName`、`tagNameEn`；SDK 收到 `openWeAgentCUI` 调用请求后需要调用 `getWeAgentTabInfo` 刷新字段，由设置标题逻辑按客户端语言环境刷新标题，并保存本次入参中的 `switchAssistantUri`。
...
2. 从当前助理详情缓存中读取 `tagName`、`tagNameEn` 并按原字段名返回。
3. 若 `tagName` 为空、缺失或读取异常，返回固定值 `"助手"`。
4. 若 `tagNameEn` 为空、缺失或读取异常，返回固定值 `"Agent"`。
5. 接口不判断客户端语言环境，语言环境判断只在设置助理 Tab 页导航栏标题时执行。
6. 接口必须使用 `try/catch` 包裹缓存读取和字段兜底逻辑：
...
7. `getWeAgentTabInfo` 只读取本地当前助理详情缓存，不发起服务端请求，不修改缓存。
...
3. 助理 Tab 页导航栏标题：
   ...
   - 冷启动时再调用 `getWeAgentTabInfo` 获取 `tagName`、`tagNameEn`；
   ...
   - 返回 `tagName`、`tagNameEn` 后，由设置标题的逻辑判断客户端语言环境：中文使用 `tagName`，英文使用 `tagNameEn`；由于 `getWeAgentTabInfo` 已保证两个字段都有值，设置标题时不再重复兜底；
...
5. IM 页面点击助理 item 项打开助理 Tab：
   ...
   - SDK 收到 `openWeAgentCUI` 调用请求后调用 `getWeAgentTabInfo`；
   - 返回 `tagName`、`tagNameEn` 后按客户端语言环境设置助理 Tab 页导航栏标题，并保存本次 `switchAssistantUri`。

### 4.3 兼容与边界

1. 助理详情页、切换助理页、选择助理页展示 tag 标签时，优先取 `tagName`、`tagNameEn`；取不到时走目前已有的 tag 展示逻辑。
2. 低版本 WeLink 升级到新版本 WeLink 时，旧版 `current_we_agent_detail` 缓存没有 `tagName`、`tagNameEn`，调用 `getWeAgentTabInfo` 将返回 `{ tagName: "助手", tagNameEn: "Agent" }`，设置标题时再按客户端语言环境选择展示值。
3. 服务端灰度期间只有部分助理返回 `tagName`、`tagNameEn` 时，已返回字段的助理展示新标签，未返回字段的助理走旧逻辑或兜底。
4. 语言环境读取失败时，设置标题逻辑默认按中文环境处理，使用 `tagName`；`tagName` 已由 `getWeAgentTabInfo` 保证有值。
5. `tagName` 或 `tagNameEn` 为空字符串时，按缺失处理。
6. `getWeAgentTabInfo` 不请求服务端，因此不会因为旧缓存缺字段阻塞助理 Tab 首屏。
7. 切换助理时如果 `getWeAgentDetails` 未成功写入 `current_we_agent_detail`，则 SDK 在收到 `openWeAgentCUI` 后调用 `getWeAgentTabInfo` 仍可能读取旧缓存或返回兜底字段；页面应以 `getWeAgentDetails` 成功和 `openWeAgentCUI` 调用成功作为刷新标题的前置条件。
...
6. `openWeAgentCUI`：切换助理页或 `openWeAgent` 内部按 `小程序JSAPI接口文档.md` 传入 `weAgentUri`、`assistantDetailUri`、`switchAssistantUri`；SDK 收到调用请求后触发 `getWeAgentTabInfo` 获取最新 `tagName`、`tagNameEn`，设置标题时按客户端语言环境选择展示值，并将 `switchAssistantUri` 作为助理 Tab 标题点击时的切换助理页面地址。
7. `getWeAgentTabInfo`：新增 SDK 本地接口，只读 `current_we_agent_detail` 并返回 `tagName`、`tagNameEn`。

### 4.5 文档需要同步修改的内容

1. `SkillClientSdkInterfaceV2.md`：补充 `tagName`、`tagNameEn` 字段定义，新增 `getWeAgentTabInfo` 接口定义与双字段兜底规则。
...
3. SDK 收到 `openWeAgentCUI` 调用请求后新增一次本地接口调用，用于读取 `tagName`、`tagNameEn`，不增加服务端压力。
...
1. `we_agent_tab_info_get`
   - 说明：记录 `getWeAgentTabInfo` 调用结果，建议包含 `success`、`tagNameFallbackUsed`、`tagNameEnFallbackUsed`、`hasTagName`、`hasTagNameEn`。
...
2. 旧缓存升级场景下，`getWeAgentTabInfo` 会返回兜底后的 `tagName`、`tagNameEn`，助理 Tab 导航栏标题可能先展示兜底值，待后续详情缓存刷新后再展示服务端下发名称。
3. 多语言切换后，已获取的 `tagName`、`tagNameEn` 可复用；设置标题逻辑需要重新按最新语言环境选择标题，必要时重新调用 `getWeAgentTabInfo` 刷新字段。
...
6. SDK 冷启动后先调用 `getWeAgentUri` 获取 `switchAssistantUri`，再调用 `getWeAgentTabInfo`，当前详情缓存包含 `tagName`、`tagNameEn` 时，返回 `{ tagName, tagNameEn }`；页面在中文环境使用 `tagName` 设置导航栏标题，并保存 `switchAssistantUri`。
7. SDK 冷启动后先调用 `getWeAgentUri` 获取 `switchAssistantUri`，再调用 `getWeAgentTabInfo`，当前详情缓存包含 `tagName`、`tagNameEn` 时，返回 `{ tagName, tagNameEn }`；页面在英文环境使用 `tagNameEn` 设置导航栏标题，并保存 `switchAssistantUri`。
8. 当前详情缓存缺少 `tagName` 时，`getWeAgentTabInfo` 返回 `{ tagName: "助手", tagNameEn }`，保证中文标题可直接使用 `tagName`。
9. 当前详情缓存缺少 `tagNameEn` 时，`getWeAgentTabInfo` 返回 `{ tagName, tagNameEn: "Agent" }`，保证英文标题可直接使用 `tagNameEn`。
10. `getWeAgentTabInfo` 读取缓存异常时，catch 中返回 `{ tagName: "助手", tagNameEn: "Agent" }`；语言环境异常时由设置标题逻辑默认按中文环境使用 `tagName`。
11. 点击助理 Tab 页导航栏标题时，优先使用冷启动阶段保存的 `switchAssistantUri` 打开切换助理页面；未保存或为空时，调用 `getWeAgentUri` 并使用返回的 `switchAssistantUri` 打开切换助理页面。
12. 切换助理页面点击其他助理后，先调用 `getWeAgentDetails({ partnerAccount })` 获取当前助理详情并写入 `current_we_agent_detail`；再调用 `openWeAgentCUI({ weAgentUri, assistantDetailUri, switchAssistantUri })`；SDK 收到 `openWeAgentCUI` 调用请求后调用 `getWeAgentTabInfo`，返回 `tagName`、`tagNameEn`，设置标题逻辑按客户端语言环境刷新助理 Tab 页导航栏标题，并保存 `switchAssistantUri` 作为点击标题时的切换助理页面地址。
13. IM 页面点击助理 item 项后，调用 `openWeAgent({ partnerAccount })`；SDK 获取目标助理详情并写入 `current_we_agent_detail`，组装 URI 并调用 `openWeAgentCUI`；SDK 收到 `openWeAgentCUI` 调用请求后调用 `getWeAgentTabInfo`，返回 `tagName`、`tagNameEn`，设置标题逻辑按客户端语言环境设置助理 Tab 页导航栏标题，并保存 `switchAssistantUri` 作为点击标题时的切换助理页面地址。

### 9.2 兼容测试

1. 服务端未返回 `tagName`、`tagNameEn` 时，详情页、切换助理页、选择助理页继续走现有 tag 展示逻辑。
2. 低版本 WeLink 升级到新版本 WeLink，旧 `current_we_agent_detail` 缓存缺少 `tagName`、`tagNameEn` 时，`getWeAgentTabInfo` 返回 `{ tagName: "助手", tagNameEn: "Agent" }`，中文环境助理 Tab 导航栏显示 `"助手"`，英文环境显示 `"Agent"`。
3. 服务端只返回 `tagName` 不返回 `tagNameEn` 时，`getWeAgentTabInfo` 返回服务端 `tagName` 和兜底 `tagNameEn: "Agent"`，英文环境可直接显示 `"Agent"`。
4. 服务端只返回 `tagNameEn` 不返回 `tagName` 时，`getWeAgentTabInfo` 返回兜底 `tagName: "助手"` 和服务端 `tagNameEn`，中文环境可直接显示 `"助手"`。
5. Android、iOS、HarmonyOS 三端 `getWeAgentTabInfo` 出参字段名统一为 `tagName`、`tagNameEn`。
...
建议采用“服务端字段扩展 + SDK 透传缓存 + `getWeAgentTabInfo` 返回兜底后的双字段 + 设置标题时按语言消费”的方案。该方案不新增服务端请求，不改变现有助理打开和切换链路，只在现有详情、列表和当前助理缓存上扩展字段，并由 `getWeAgentTabInfo` 用固定兜底值保证旧缓存和异常场景下的导航栏可用性。后续优先补齐 `SkillClientSdkInterfaceV2.md` 接口定义，再推进三端 SDK 与 `ai-chat-viewer` 页面联调。
