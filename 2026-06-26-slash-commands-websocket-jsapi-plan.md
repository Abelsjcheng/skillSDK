...
当前 `ai-chat-viewer` 已有 slash command 联想相关前端设计与实现痕迹，但已有方案基于 `HWH5.fetch` 请求 HTTP 接口获取命令列表。新的需求是：Skill SDK 向 JSAPI bridge 提供一个发送 WebSocket message 的方法，该方法不是普通 REST 接口，而是通过既有 WebSocket 长连接发送命令：
...
```mermaid
flowchart TD
    A["小程序 / PC 页面"] --> B["JSAPI bridge: sendWebSocketMessage"]
    B --> C["Skill SDK 参数校验"]
    C --> D{"WebSocket 已连接?"}
    D -->|是| E["WebSocketManager 发送通用 JSON message"]
...
| 方案 | 描述 | 优点 | 缺点 | 结论 |
|------|------|------|------|------|
| 方案 A：通用发送，结果走 `registerSessionListener` | `sendWebSocketMessage` 只保证 WebSocket message 发送成功，业务结果由对应事件异步返回 | 复用现有事件流；三端改动小；符合 WebSocket 推送模型；后续新增 action 不需要改 SDK API | 调用方需要先注册 listener；Promise resolve 不代表已经拿到业务结果 | 推荐 |
| 方案 B：为每个 action 新增专用 SDK 方法 | 例如 `querySlashCommands`、后续再新增其他 action 方法 | 单个业务调用语义清晰 | 每新增 action 都要改三端 SDK 和 JSAPI，扩展成本高 | 不推荐 |
| 方案 C：继续使用 REST / `HWH5.fetch` | 页面或 bridge 调 HTTP 接口查询命令列表 | 实现简单；请求-响应语义清晰 | 不符合本次“通过 WebSocket 发送 message”的需求；与已有长连接能力割裂 | 不采用 |
...
5. `stopSkill` / `closeSkill`：会话关闭后调用 `sendWebSocketMessage` 应返回会话不可用或发送失败错误，具体错误码待服务端和 SDK 统一。
...
新增一次 WebSocket message 发送和一次服务端推送，不新增 HTTP 请求，不新增额外长连接。命令列表大小通常较小，对传输与解析影响有限。高频输入 `/` 时不建议 SDK 层自动查询，应由页面侧做触发控制、缓存或节流，避免反复向服务端发送 `query_slash_commands`。
...
1. `websocket_message_send`
   - 说明：SDK 成功发送 WebSocket message 时上报，可包含 `action`、平台、耗时。
2. `websocket_message_error`
...
最终结论：推荐采用“`sendWebSocketMessage` 只负责发送通用 WebSocket message，业务结果仍通过 `registerSessionListener` 返回”的方案。该方案复用现有 WebSocketManager、连接管理、会话分发和 JSAPI 回调模型，三端改动较小，也避免为 WebSocket 异步推送额外设计一套 request-response 超时匹配机制。
