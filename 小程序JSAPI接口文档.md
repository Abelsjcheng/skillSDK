...
| [getWeAgentList](#12-getweagentlist) | 查询个人助理列表（同步返回缓存） |
| [getWeAgentDetails](#13-getweagentdetails) | 获取并持久化助理详情 |
| [createNewSession](#14-createnewsession) | 创建新会话 |
| [getHistorySessionsList](#15-gethistorysessionslist) | 获取历史会话列表 |
| [openWeAgentCUI](#16-openweagentcui) | 打开助理 CUI |
...
返回 Promise，resolve 数据：`AgentType[]`
...
```javascript
window.HWH5EXT.getAgentType().then((list) => {
  list.forEach((item) => {
...
```javascript
const list = window.HWH5EXT.getWeAgentList(params)
```
...
根据 `aiChatViewInterface.md`，对应的 PC 端直连接口如下：

```javascript
// 1) 从服务端查询个人助理列表
const res = await window.Pedestal.callMethod('method://agentSkills/getWeAgentList', {
  pageSize: params.pageSize,
  pageNumber: params.pageNumber
});

// 2) 从 DB 读取个人助理列表缓存（key: WeAgentList）
const cacheString = await window.Pedestal.callMethod('method://agentSkills/saveDb', {
  type: 'query',
  params: {
    key: 'WeAgentList'
  }
...
// 3) 向 DB 写入个人助理列表缓存（key: WeAgentList）
await window.Pedestal.callMethod('method://agentSkills/saveDb', {
  type: 'add',
  params: {
    key: 'WeAgentList',
...
1. 接口同步返回本地 SP 缓存（无缓存时返回空数组）。
2. 本地缓存读取不按分页参数读取；如需分页展示，可在内存中切片处理。
3. SDK 在后台异步请求服务端 `GET /v4-1/we-crew/list` 刷新缓存。
4. 缓存按 `userId` 隔离（当前 `userId` 使用 mock 值）。

### 调用示例

```javascript
const list = window.HWH5EXT.getWeAgentList({
  pageSize: 10,
  pageNumber: 1
});
console.log('当前缓存列表长度:', list.length);
```
...
根据 `partnerAccount` 获取指定助理详情。

调用成功后，SDK 会将该助理详情持久化到 SP（`current_we_agent_detail`，按 `userId` 隔离）。
...
```javascript
// 1) 从服务端获取助理详情
const detail = await window.Pedestal.callMethod('method://agentSkills/getWeAgentDetails', {
  partnerAccount: params.partnerAccount
});

// 2) 从 DB 读取当前助理详情缓存（key: currentWeAgentDetails）
const cacheString = await window.Pedestal.callMethod('method://agentSkills/saveDb', {
  type: 'query',
  params: {
...
// 3) 向 DB 写入当前助理详情缓存（key: currentWeAgentDetails）
await window.Pedestal.callMethod('method://agentSkills/saveDb', {
  type: 'add',
  params: {
    key: 'currentWeAgentDetails',
...
返回 Promise，resolve 数据：`WeAgentDetails`
...
| assistantDetailUri | string | 是 | 助理详情地址（`h5://123456/html/index.html` 并追加 `partnerAccount` query） |
| switchAssistantUri | string | 是 | 切换助理地址（`h5://123456/html/index.html` 并追加 `partnerAccount` query） |
