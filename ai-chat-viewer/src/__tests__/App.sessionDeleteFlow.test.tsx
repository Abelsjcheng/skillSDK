...
describe('App session delete flow', () => {
  ...
  it('creates a fallback session when the local delete action removes the last current session', async () => {
    ...
    expect(latestHistorySidebarProps?.cachedCache?.content.map((session) => session.welinkSessionId)).toEqual([
      'fallback-1',
    ]);
  });
  ...
  it('keeps the locally created fallback in cache when the delete push echo arrives', async () => {
    render(<App assistantAccount="assistant-1" />);
