...
- 方案日期：`2026-06-18`
- 目标工程：`ai-chat-viewer`
- 参考文档：`src/pages/weAgentCUI.tsx`、`src/App.tsx`、`src/utils/hwext.ts`、`src/utils/versionCheck.ts`、`src/types/bridge/hwext.ts`、`src/styles/WeAgentCUI.less`、`src/styles/WeAgentCUIFooter.less`
...
3. 底部输入区可能被鸿蒙分栏或底部安全区遮挡，需要优先使用 CSS 原生安全区变量 `env(safe-area-inset-bottom, 0px)`，并使用 `deviceInfo.safeAreaInsetBottom` 作为渐进补充。
...
3. 顶部安全区优先使用项目已有 `--mobile-safe-top: env(safe-area-inset-top, 0px)`；`deviceInfo.statusBarHeight` 表示宿主补充的顶部状态栏高度。
4. 底部安全区新增 `--mobile-safe-bottom: env(safe-area-inset-bottom, 0px)`；`deviceInfo.safeAreaInsetBottom` 表示宿主补充的底部安全区高度。
5. 鸿蒙分栏特性最低支持 Harmony `1.30.0`，通过 `canIUse.harmonySplitLayout()` 统一做版本能力判断。

### 1.2 需求目标

1. 仅在 `weAgentCUI` 页面、鸿蒙移动端、分栏状态下展示固定标题栏。
2. 标题栏标题读取当前助手名称，即 `App` 中已获取到的 `weAgentAssistantName`，居中显示。
3. 标题栏顶部优先通过 `--mobile-safe-top` 避让系统安全区；当 `deviceInfo.statusBarHeight > 0` 时使用宿主返回值覆盖局部变量，避免被系统状态栏图标覆盖。
4. 标题栏固定在页面顶部，消息区滚动时标题栏不随消息滚走。
5. 底部输入区优先通过 `--mobile-safe-bottom` 避让底部安全区；当 `deviceInfo.safeAreaInsetBottom > 0` 时使用宿主返回值覆盖局部变量，避免被底部安全区遮挡。
6. 标题栏内容高度为 `44px`，由于 `.app-container--we-agent-cui` 已有 `padding: 8px 16px 12px`，顶部有效避让高度为 `max(顶部安全区高度 - 8px, 0px)`，总占位高度为 `44px + 顶部有效避让高度`；标题文字样式为 `font-size: 16px`、`font-weight: 500`、亮色模式 `color: #333`。
...
flowchart TD
    ...
    C --> D{"非 PC 且 osType === Harmony 且 isFullScreen === 0"}
    D -- 否 --> E["保持现有 weAgentCUI 布局"]
    D -- 是 --> V{"canIUse.harmonySplitLayout()"}
    V -- 否 --> E
    V -- 是 --> F["记录 statusBarHeight / safeAreaInsetBottom"]
...
在 `App` 内新增 `weAgentCUI` 专用的鸿蒙分栏布局状态：通过 `getDeviceInfo()` 判断 `Harmony + isFullScreen === 0`，再通过 `canIUse.harmonySplitLayout()` 校验 Harmony `1.30.0` 最低版本后，用 class 和 CSS 变量驱动固定标题栏、顶部内容让位和底部安全区避让，业务接口与其它端保持不变。
...
sequenceDiagram
    ...
    App->>App: canIUse.harmonySplitLayout() 校验 Harmony >= 1.30.0
    App->>API: getWeAgentDetails({ partnerAccount })
    API-->>App: 返回助手详情
    App->>App: 设置 weAgentAssistantName
    App->>App: 条件渲染固定标题栏；CSS env 先兜底，deviceInfo 正数时覆盖局部安全区变量
```
...
sequenceDiagram
    ...
    App->>Content: 使用标题栏高度和顶部安全区增加顶部让位
    App->>Footer: 使用底部安全区增加底部让位
    Footer-->>User: 输入区保持可见
```
...
3. 在 `src/App.tsx` 中新增移动端鸿蒙分栏布局状态，例如 `harmonySplitLayout`：
   ...
   - `statusBarHeight: number`
   - `safeAreaInsetBottom: number`
4. 在 `src/utils/versionCheck.ts` 中定义鸿蒙分栏特性的最低版本：
   - `HARMONY_SPLIT_LAYOUT_MIN_VERSION = { harmony: '1.30.0' }`
   - 通过 `canIUse.harmonySplitLayout()` 暴露给页面层使用。
5. 在 `App` 初始化阶段调用 `getDeviceInfo()`，仅当 `!isPc && deviceInfo.osType === 'Harmony' && deviceInfo.isFullScreen === 0 && await canIUse.harmonySplitLayout()` 时启用布局优化。
6. 在 `src/styles/theme.less` 中补充底部原生安全区变量：
   - `--mobile-safe-bottom: env(safe-area-inset-bottom, 0px)`
7. 在 `App` 根节点追加条件 class，例如 `is-harmony-split`，并通过 inline CSS variables 做渐进补充：
   - `--we-agent-cui-status-bar-height`
   - `--we-agent-cui-safe-area-bottom`
   - `--we-agent-cui-title-bar-height`
   - 其中顶部、底部高度仅在 `deviceInfo` 返回正数时写入；否则保持 CSS 默认值 `var(--mobile-safe-top)` / `var(--mobile-safe-bottom)`
8. 在 `we-agent-cui-chat-panel` 内、`content-wrapper` 之前条件渲染标题栏：
   - class 建议为 `we-agent-cui-titlebar`
   - 文案使用 `weAgentAssistantName || ''`
   - 使用 `aria-label` 或 `role` 保持可访问性
9. 在 `src/styles/WeAgentCUI.less` 中新增 `.app-container--we-agent-cui.is-harmony-split:not(.pc-mode)` 样式，控制固定标题栏、消息区顶部让位、底部安全区避让和暗黑模式颜色。

### 4.2 核心实现方式

推荐在 `App` 中集中完成鸿蒙分栏判断，样式层优先使用 CSS 原生安全区变量，再由 `deviceInfo` 正数结果覆盖局部变量，避免将鸿蒙分栏逻辑扩散到 `weAgentCUI.tsx`、`Content`、`WeAgentCUIFooter` 等子组件。
...
const isHarmonySplitLayoutSupported =
  isHarmonySplit && await canIUse.harmonySplitLayout();
```
...
2. `top: 0`，内部使用 `padding-top: var(--we-agent-cui-effective-status-bar-height)` 避让状态栏；`--we-agent-cui-status-bar-height` 默认取 `var(--mobile-safe-top)`，宿主返回正数 `statusBarHeight` 时覆盖；`--we-agent-cui-effective-status-bar-height` 取 `max(calc(var(--we-agent-cui-status-bar-height) - 8px), 0px)`，扣除容器已有顶部 padding。
...
2. `.we-agent-cui-bottom` 在鸿蒙分栏下增加 `padding-bottom: var(--we-agent-cui-effective-safe-area-bottom)`；`--we-agent-cui-safe-area-bottom` 默认取 `var(--mobile-safe-bottom)`，宿主返回正数 `safeAreaInsetBottom` 时覆盖；`--we-agent-cui-effective-safe-area-bottom` 取 `max(calc(var(--we-agent-cui-safe-area-bottom) - 12px), 0px)`，扣除容器已有 `12px` 底部 padding。
...
4. 鸿蒙版本边界：`canIUse.harmonySplitLayout()` 返回 `false` 时不启用本方案，Harmony 低于 `1.30.0` 保持现有布局。
5. 宿主字段缺失边界：`osType` 缺失或 `isFullScreen` 缺失时不启用鸿蒙分栏布局；`statusBarHeight`、`safeAreaInsetBottom` 非数字或为 0 时不覆盖局部变量，继续使用 CSS 原生 `env(safe-area-inset-top/bottom)` 默认值。
6. 助手名称为空边界：标题栏仍可渲染固定区域，但标题文案为空；也可按评审结论降级展示 `t('weAgent.title')`，当前推荐不新增文案以避免与“读取当前助手名称”需求冲突。
7. 历史侧边栏边界：移动端历史侧边栏打开时，其遮罩和面板应覆盖标题栏；如现有层级不足，需调整 sidebar z-index 高于标题栏。
8. 暗黑模式边界：现有 `@media (prefers-color-scheme: dark)` 中 `.app-container--we-agent-cui:not(.pc-mode)` 已切换为 `var(--ai-dark-page-bg)`，新增标题栏和底部安全区必须同源使用该变量；标题文字颜色需覆盖为 `var(--ai-dark-text-primary)` 或等价暗黑主文本色，避免沿用亮色 `#333`。
9. iOS 键盘适配边界：`useIosKeyboardLift` 仅用于 iOS，鸿蒙分栏样式变量与其 style 合并，不改变原有 hook 行为。
...
5. `canIUse.harmonySplitLayout()`：
   - 来源：`src/utils/versionCheck.ts`
   - 用途：判断当前客户端是否支持鸿蒙移动端分栏标题栏与安全区适配
   - 变更：新增 Harmony `1.30.0` 最低版本配置
...
1. `src/App.tsx`：新增鸿蒙分栏布局状态、条件标题栏渲染、根节点 class、版本能力判断和宿主正数高度补充变量。
...
5. `src/styles/theme.less`：新增 `--mobile-safe-bottom: env(safe-area-inset-bottom, 0px)`。
6. `src/utils/versionCheck.ts`：新增 `canIUse.harmonySplitLayout()`，Harmony 最低支持版本为 `1.30.0`。
...
1. 鸿蒙移动端分栏打开 `weAgentCUI`，确认顶部出现固定标题栏，标题为当前助手名称且居中显示；标题栏内容高度为 `44px`，顶部额外避让为 `max(顶部安全区高度 - 8px, 0px)`，其中顶部安全区优先来自 `env(safe-area-inset-top)`，宿主返回正数时来自 `deviceInfo.statusBarHeight`。
2. 鸿蒙移动端分栏下滚动长消息列表，确认标题栏固定在顶部，不随消息滚动消失。
3. 鸿蒙移动端分栏下首条消息、欢迎区、加载更多区域不被标题栏遮挡。
4. 鸿蒙移动端分栏下底部输入框、发送按钮、停止按钮不被底部安全区遮挡，底部额外避让为 `max(底部安全区高度 - 12px, 0px)`，其中底部安全区优先来自 `env(safe-area-inset-bottom)`，宿主返回正数时来自 `deviceInfo.safeAreaInsetBottom`。
5. 鸿蒙移动端分栏下切换历史会话、新建会话、发送消息、停止生成流程正常。
6. 助手名称异步加载前后，标题栏文案能从空态更新为当前助手名称。
7. 亮色模式确认标题文字为 `font-size: 16px`、`font-weight: 500`、`color: #333`。
8. Harmony `1.29.9` 调用 `canIUse.harmonySplitLayout()` 返回 `false`，Harmony `1.30.0` 返回 `true`。
...
最终结论：推荐采用 `App` 集中判断 + `canIUse` 版本门禁 + CSS 变量驱动的轻量方案。该方案复用现有 `getDeviceInfo()`、`canIUse` 和 `weAgentAssistantName`，不新增业务接口，不改动三端 SDK 协议和消息链路；通过 `Harmony + isFullScreen === 0 + !pc-mode + Harmony >= 1.30.0` 四重条件控制影响范围，可以精确满足鸿蒙移动端分栏适配，同时把 PC、Android、iOS、低版本鸿蒙和其它页面的回归风险降到最低。
