...
| 点击 | `weagent_send_message_click` | WeAgentCUI / skillCUI 点击发送消息；仅 WeAgentCUI 额外带助理标识 | `{ entry, // 固定 WeAgent operationTime, // 操作时间戳 page: 'weAgentCUI' \| 'skillCUI', // 页面 welinkSessionId, // 当前会话 id robot_Id, // 仅 WeAgentCUI：助理/机器人标识 bizRobotId, // 仅 WeAgentCUI：业务机器人标识 contentLength // 消息长度，不上报原文 }` |
