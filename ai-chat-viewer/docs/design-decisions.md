...
17. 为保证输入交互和校验规则一致，第一页表单输入控件本身也直接添加原生长度上限：名称输入框 `maxLength=50`，简介输入框 `maxLength=250`；校验层同步保持名称最大 `50`、简介最大 `250`。
18. 操作区使用 `padding: 16px 24px 12px`，右对齐两个按钮：取消/下一步。
19. 操作区必须固定在组件容器可视范围内，不得超出底部边界；内容区域应自适应可滚动。
20. 操作区按钮文本统一样式为 `font-size: 12px; line-height: 20px`。
21. “下一步”启用条件：`name.trim() && description.trim()`。
...
5. 助理列表项样式固定：
   ...
   - 列表项最左侧新增一个 `24x24` 单选按钮容器，距离列表项左边固定 `12px`；单选按钮与头像之间固定 `16px` 间距。
   - 单选按钮未选中态使用 `src/imgs/no_selection_icon.svg`，选中态使用 `src/imgs/selection_icon.svg`。
   - 左侧头像 `40x40`，统一显示为圆形头像，与右侧说明块间距 `16px`。
   - 列表项支持单选态，但选中反馈只通过左侧单选按钮图标切换体现；列表项本身不再增加蓝色高亮边框。
   - 列表项去除浏览器默认点击/聚焦高亮，不再额外叠加边框或阴影反馈。
...
17. 由于“切换助理页”与“启动助理页”共用 `AssistantCardList`，左侧单选按钮图标方案也统一收口到共享列表组件与 `SwitchAssistant.less` 中实现，两个页面不分别维护选中态 DOM 结构与样式。
...
10. 启动助理页复用共享助理列表时，同样使用左侧单选按钮图标表达选中态；列表项边框不做选中高亮，仅保留白底卡片本身。为避免影响切换助理页与移动端，共享组件保持默认 `24x24`，仅在 PC 端启动助理页作用域内把单选按钮尺寸覆写为 `16x16`。
...
27. `WeAgentCUI` 的 `QuestionCard` 统一按对象化选项模型渲染：`options` 在进入 UI 前归一化为 `{ label, description? }[]`；渲染时按钮主文案展示 `label`，存在 `description` 时在下方展示辅助说明，同时兼容字符串数组输入并转换为仅含 `label` 的对象项。`question` 渲染默认只读取顶层 `questions`、`header`、`question`、`options`、`multiSelect` 字段；仅历史消息在顶层 `questions` 缺失时允许从 `input.questions` 兜底取题目数组，实时、snapshot、发送返回消息不从 `input.questions` 或 `input.options` 解析题目与选项。历史消息的已回答展示只读取最外层 `output`：`string[][]` JSON 按题目顺序映射，普通字符串只作为第一题答案展示，忽略 `questions` item 内的 `output` 字段。选项区域改为纵向单列布局，每个问题选项块独占一行。选项按钮 hover 态仅允许背景或边框变化，主文案与说明文案颜色保持默认值，不随 hover 切换为白色。参考 `skill-miniapp`，`QuestionCard` 不在组件内部直接调用 `sendMessage`，而是只把 `answer + toolCallId` 上抛给 `App`；`App` 统一复用现有用户发送链路插入独立用户消息，提交回答时单题单答案复用旧答案字符串，多题、多选或多个答案发送二维数组 JSON 字符串，并让后续 AI 回复继续走常规流式助手消息。用户回答消息气泡只展示答案文本，多题按行展示答案，不展示 `第1题:`、`第2题:` 等题目前缀。单题单选点击 option 仍立即提交；填写自定义答案时不再使用输入框尾部发送图标，改为显示底部提交按钮。为避免 question 完成事件在流式态结束后又重新生成一个 question 消息块，监听层需在 `question completed/error` 且当前无活跃流式 question 消息时，仅补丁更新原 `QuestionCard` 的回答状态与结果。
...
28. `PermissionCard` 改为“头部 / 内容 / 结果或操作区”三段式结构：
   - `permission-card` 主体保留 `1px solid rgba(238,238,238,1)` 边框与 `8px` 圆角；头部固定高 `40px`、`padding: 0 12px`、背景 `rgba(245,245,245,1)`，左侧锁图标使用 `src/imgs/lock_icon.png`，图标容器尺寸 `20px x 20px`、图片本身尺寸 `16px x 16px`，右侧 `4px` 显示 `typeLabel`；
...
29. `ToolCard` 统一收敛为“浅灰头部 + 白色内容区”的信息卡结构：
   - 外层只保留 `1px solid rgba(238,238,238,1)` 边框与 `8px` 圆角，不再通过不同状态切换左侧高亮边条；
   - 头部固定高 `40px`、`padding: 0 12px`、背景 `rgba(245,245,245,1)`，状态图标统一放进 `20px x 20px` 容器，图标本体为 `16px x 16px`；
...
6. `hwext.ts` 内统一新增 `reportUemEvent(eventId, eventTitle, data)` 运行时封装，业务层不直接访问 `window.HWH5.uem`：
   - 移动端固定调用 `window.HWH5.uem('event', payload)`，其中 `payload` 结构收敛为 `{ type: 'info', code, name, result: true, msg, duration: 0, data }`；
   - PC 端先仅保留 `TODO` 分支，不提前猜测或硬编码 PC 埋码桥协议。
7. 业务层点击埋码统一采用“调用即放行”的接入方式：点击事件内只执行 `void reportUemEvent(...).catch(...)`，不 `await` 埋码结果，也不因为埋码失败中断原有按钮行为。当前固定触发点包括：
   - `activate_select_assistant_click` / `选择助理`
   - `weagent_history_click` / `历史会话`
   - `weagent_create_session_click` / `创建会话`
   - `switch_assistant_confirm_click` / `确认切换`
