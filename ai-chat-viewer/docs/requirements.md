...
5. “名称”输入校验规则：
   - 仅允许：汉字 / 字母 / 数字；
   - 长度限制：`2-50` 个字符，输入框最多允许输入 `50` 个字符。
6. “简介”输入校验规则：
   - 仅允许：汉字 / 字母 / 数字 / 常用标点符号；
   - 常用标点包含中英文常见句读（如 `，。！？；：、,.!?;:()【】《》“”‘’` 等）；
   - 长度限制：`2-250` 个字符，输入框最多允许输入 `250` 个字符。
...
3. 内容区：
   ...
   - 单个助理块样式：
      ...
      - 最左侧新增单选按钮，尺寸固定 `24px x 24px`，距离列表项左边 `12px`；
      - 单选按钮右侧 `16px` 为助理头像图标，头像尺寸固定 `40px x 40px`，显示为圆形头像；
      - 头像右侧间距 `16px` 为助理说明块；
      - 未选中时单选按钮显示 `src/imgs/no_selection_icon.svg`；
      - 选中时单选按钮显示 `src/imgs/selection_icon.svg`。
   - 点击助理块后，仅通过左侧单选按钮图标切换体现当前选中态；列表项边框不再做蓝色高亮，也不显示额外背景高亮、系统点击高亮或焦点发光。
...
9. 列表数据渲染约束：
   ...
   - 选择助理页面中的助理头像统一显示为圆形头像；
   - 助理列表项左侧统一新增单选按钮，距离列表项左边 `12px`、距离头像 `16px`；未选中使用 `src/imgs/no_selection_icon.svg`，选中使用 `src/imgs/selection_icon.svg`；
   - 其中 PC 端“启动助理页面”的单选按钮尺寸单独覆盖为 `16px x 16px`；
   - 当前选中态仅通过左侧单选按钮图标体现，列表项边框不再高亮；
...
2. 整体布局：
   - 移动端页面根容器内边距：上 `8px`、左右 `16px`、下 `12px`（`padding: 8px 16px 12px`）。
...
4. 用户消息块：
   ...
   - 文本样式：`12px / 400 / 20px`，颜色 `rgba(102,102,102,1)`，示例：`测试 2026-04-13 08:07`。
...
5. AI 消息块：
   ...
   - 文本样式：`12px / 400 / 20px`，颜色 `rgba(102,102,102,1)`，示例：`小米 2026-04-13 08:07`。
   - 头像尺寸：`24px x 24px`。
...
7. 底部输入区：
   - 宽度占满，高度 `40px`，内边距 `8px 12px`，圆角 `30px`。
   - 默认态无边框，背景颜色为 `rgba(255, 255, 255, 0.9)`。
   - 默认态阴影为 `0px 4px 8px -5px rgba(107, 133, 235, 0.05)`。
   - 内部左侧输入框与右侧发送按钮间距 `8px`。
   - 输入框 placeholder：`有问题尽管问我~`。
   - 输入框容器聚焦后背景仍保持 `rgba(255, 255, 255, 0.9)`，并显示 `1px` 渐变边框；边框方向固定为从左到右，输入框左侧边框及左上/左下圆角处为 `#0d92ff`，沿水平方向平滑渐变到右侧边框及右上/右下圆角处的 `#5145ff`。
   - 移动端点击输入框前后，输入框容器背景色视觉需保持完全一致，不允许因聚焦态仅为了显示渐变边框而让整体背景出现额外加深、变灰或渐变铺底效果。
   - 渐变边框实现不再使用 `border-image`，也不再让渐变直接参与输入框主体背景绘制；改为使用伪元素渲染独立的“空心渐变描边”层，主体背景继续单独保持 `rgba(255, 255, 255, 0.9)`，保证圆角与半透明背景稳定显示。
   - 发送按钮尺寸 `24px x 24px`，发送图标尺寸 `20px x 20px`。
   - 移动端点击输入框 `focus` 时，需立即触发对话内容区滚动到底部，确保最新消息区域与底部输入框一并进入可视范围，尽量避免被软键盘遮挡。
...
8. 当对话内容区消息为空时，显示欢迎块：
   ...
   - PC 端欢迎块单独覆盖样式：
      - 欢迎块距离顶部 `57px`；
      - 标题距离头像块 `16px`，标题样式 `28px / 600 / 30px`；
      - 副标题距离标题 `24px`，副标题样式 `14px / 400 / 24px`；
...
10. 历史会话侧边栏：
   ...
   - 历史会话侧边栏顶部新增标题区：高度 `32px`，宽度占满；标题区位于上述弹窗容器内部；标题容器 `padding: 6px 12px`；移动端标题文本内容为“历史对话”，样式 `16px / 500 / 24px`、`rgba(51,51,51,1)`；PC 端标题文本保持 `12px / 500`；
   - 侧边栏保持可滚动，但需隐藏可视滚动条。
11. 历史会话数据获取：
...
20. `WeAgentCUI` 中渲染 AI `question` 类型消息块时：
   ...
   - 用户回答消息气泡只展示答案文本，多题按行展示答案，不展示 `第1题:`、`第2题:` 等题目前缀；
   ...
   - 历史消息的已回答展示只读取最外层 `output`：`string[][]` JSON 按题目顺序映射，普通字符串只作为第一题答案展示，忽略 `questions` item 内的 `output` 字段；
...
7. 业务页面、组件不再直接散落调用 `reportUemEvent(...)`；需在 `src/utils` 下新增 `uemUtil.ts`，集中封装上述点击埋码的具名方法后再对外导出使用。
...
4. 选择助理/切换助理页面：
   ...
   - 助理选中态仅通过左侧单选图标资源体现，不再使用列表项边框高亮。
5. 助理详情页面：
...
8. WeAgentCUI 页面：
   ...
   - “查看历史会话”“新建会话”两个操作按钮继续复用现有黑色 SVG 图标资源，亮色模式保持黑色显示，暗黑模式下通过 `filter` 将图标视觉近似调整为 `rgba(220,221,221,1)`；
   - 暗黑模式下代码块复制图标、权限块锁图标颜色统一为 `rgba(220,221,221,1)`；
   - 暗黑模式下 AI 消息块内各折叠/收起箭头图标颜色统一为 `rgba(174,175,176,1)`；
