...
6. 创建个人助理页面：
   ...
   - 非暗黑模式下，自定义上传头像块中的加号改为从 `src/imgs` 导入的 `add.png` 图片；
   - 暗黑模式下，自定义上传头像块中的加号改为从 `src/imgs` 导入的 `add_black.png` 图片；
   ...
   - 创建个人助理页面导航头中的返回/关闭图标在暗黑模式下统一显示为 `rgba(220,221,221,1)`。
7. 选择助理、助理详情、切换助理页面：
   - 导航头中的返回图标、客服/关闭/编辑等头像类功能图标在暗黑模式下统一显示为 `rgba(220,221,221,1)`。
8. WeAgentCUI 页面：
