...
1. 新增一个独立页面打包入口（暂记为 `personal-agent-page`），产物包含可直接访问的 `index.html` 与对应脚本资源。
2. 页面内部复用 `PersonalAgentCreator` 实现 UI 与交互，但不通过 `src/lib/index.ts` 对外导出该组件。
3. 页面内部包含 2 个页面：
...
6. 当前版本中，`X`、`取消`、`确定` 三个点击事件先空实现（no-op）。
7. 状态职责拆分：
   - 页面 1 的状态变量在 `StepBasicInfo` 内部管理；
   - 页面 2 的状态变量在 `StepBrainSelect` 内部管理；
   - `PersonalAgentCreator` 仅保留跨页面公共方法与步骤切换控制。
...
4. 任一页面点击右上角 `X` 或“取消”在当前版本中不触发流程变化（no-op）。
...
1. 页面入口需直接渲染 `PersonalAgentCreator`，不额外包裹父容器。
...
3. `PersonalAgentCreator` 内部状态：
   - `step`：页面步骤切换状态（1/2）。

### 6.2 输出行为（页面内部）

1. 不对外提供回调或事件。
2. 点击 `X` / “取消” / “确定”当前版本均为 no-op：
   - 不触发状态重置；
   - 不触发页面切换；
   - 不触发提交行为。
