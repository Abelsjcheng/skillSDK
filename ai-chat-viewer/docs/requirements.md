...
1. 标签文本：
   ...
   - 当名称输入校验失败时，在标题右侧 `8px` 显示提示文本：`仅支持汉字、数字、字母`，样式：`14px / 400`，颜色 `rgba(243,111,100,1)`
2. 下方间距 `8px` 为输入框：
...
1. 标签文本：
   ...
   - 当简介输入校验失败时，在标题右侧 `8px` 显示提示文本：`仅支持汉字数字字母/常用标点符号`，样式：`14px / 400`，颜色 `rgba(243,111,100,1)`
2. 下方间距 `8px` 为简介输入框：
...
10. 创建个人助理二维码场景：
   ...
   - 仅当“当前时间戳 > expireTime * 1000”时才判定为已过期；
...
2. 内容区：
   - 内容区整体内边距调整为：上 `12px`、左右 `16px`、下 `0`。
...
2. 内容区：
   - 内容区整体内边距调整为：上 `12px`、左右 `16px`、下 `0`。
   - `digital-twin__brain-title` 与后续内容区之间的垂直间距调整为 `20px`。
...
12. 助理头像地址兼容规则（助理详情/切换助理/启用助理）：
   - 在 `ai-chat-viewer/src/constants.tsx` 中通过方法维护 `isProEnv()`、`APP_ID()`、`HOST()`；
...
13. 新增 UAT 打包能力：
   - 在 `package.json` 中新增 `build:uat` 脚本，产物需通过 `isProEnv()=false` 进入 uat 分支；
...
1. `ai-chat-viewer` 需支持系统暗黑模式，样式切换仅通过 CSS 媒体查询 `@media (prefers-color-scheme: dark)` 实现，不新增 JS 运行时主题判断。
2. 暗黑模式下所有页面根容器背景统一为 `rgba(18,19,20,1)`。
3. 激活页面：
   - “选择助理”按钮背景改为 `linear-gradient(270deg, rgba(31,120,255,1), rgba(13,148,255,1) 100%)`。
4. 选择助理/切换助理页面：
   - 选中助理块边框颜色 `rgba(103,164,255,1)`；
   - 助理名称颜色 `rgba(220,221,221,1)`；
   - 助理简介颜色 `rgba(174,175,176,1)`；
   - 助理块背景颜色 `rgba(31,33,34,1)`；
   - tag 背景颜色 `rgba(29,63,114,1)`，tag 文本颜色 `rgba(103,164,255,1)`；
   - 底部左按钮背景颜色 `rgba(36,38,39,1)`，文本颜色 `rgba(174,175,176,1)`；
   - 底部右按钮背景颜色 `rgba(13,148,255,1)`，文本颜色白色。
5. 助理详情页面：
   - 各内容块背景颜色 `rgba(31,33,34,1)`；
   - 各 item 块标题文本颜色 `rgba(220,221,221,1)`；
   - item 值文本颜色 `rgba(174,175,176,1)`。
6. 创建个人助理页面：
   - 头像预览和默认头像背景保持 `rgba(239,246,255,1)`；
   - 自定义上传头像块边框颜色 `rgba(127,130,131,1)`，背景透明透出页面背景；
   ...
   - 暗黑模式下，头像预览块不显示边框；
   - 暗黑模式下，默认头像选中后容器 `padding: 1px`，未选中时不设置 `padding`；
   - 暗黑模式下，默认头像未选中时无边框；
   - 暗黑模式下，默认头像选中态 `padding` 区域背景为透明；
   - 暗黑模式下，默认头像选中态边框为 `1px solid rgba(13,148,255,1)`；
   - 第二页所有文本颜色 `rgba(220,221,221,1)`；
   - 第二页助理块边框颜色 `rgba(255,255,255,0.05)`，背景透明透出页面背景；
   - 第二页助理块选中后边框颜色 `rgba(13,148,255,1)`、背景颜色 `rgba(5,59,102,1)`、助理名称颜色 `rgba(13,148,255,1)`。
   - 创建个人助理页面导航头中的返回/关闭图标和标题文本在暗黑模式下统一显示为 `rgba(220,221,221,1)`。
7. 选择助理、助理详情、切换助理页面：
   - 导航头中的标题文本、返回图标、客服/关闭/编辑等功能图标在暗黑模式下统一显示为 `rgba(220,221,221,1)`。
8. WeAgentCUI 页面：
   - 所有文本颜色统一调整为 `rgba(220,221,221,1)`；
   - 欢迎块中的助理名称和简介颜色调整为 `rgba(127,130,131,1)`。
