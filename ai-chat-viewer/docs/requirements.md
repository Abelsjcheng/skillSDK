...
7. 助理详情/切换助理页面头像资源：`src/imgs/assistant-avatar.svg`、`src/imgs/switch-assistant-avatar.svg`。
...
1. 内容区：
   - 内容区容器宽度为自适应 `100%`；
   - 内容区容器高度为内容自适应；
   - 激活助理页面整体背景图使用 `src/imgs/activate-bg.svg`；
...
1. 标题区：
   - 高度 `44px`，背景白色；
   - 左侧控件尺寸 `90px x 44px`，包含两个图标按钮，均垂直居中：
      - 返回按钮：`24px x 24px`，距离左侧 `10px`，点击事件空实现；
      - 客服按钮：`24px x 24px`，距离返回按钮 `16px`，点击事件空实现；
...
1. 标题区：
   - 与“助理详情页面”标题区样式一致；
   - 高度 `44px`，背景白色；
   - 左侧控件尺寸 `90px x 44px`，包含两个图标按钮（返回、客服），均垂直居中；
