...
6. 当前版本中，`X` 与 `取消` 点击事件直接调用 `window.Pedestal.remote.getCurrentWindow().close()`；`确定` 点击事件先空实现（no-op）。
7. 状态职责拆分：
...
5. 页面 2 点击“确定”在当前版本中不触发提交流程（no-op）。
...
3. 第 5 个为自定义上传入口（`+`）：
   ...
   - 边框 `0.63px solid rgba(221,221,221,1)`
...
7. `X` 与取消点击会触发 `window.Pedestal.remote.getCurrentWindow().close()` 且不报错；确定点击不报错且当前版本行为为 no-op。
