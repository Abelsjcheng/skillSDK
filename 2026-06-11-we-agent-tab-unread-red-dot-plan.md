...
本方案将未读状态收口到服务端管控，端侧只维护当前助理的内存缓存、刷新状态并产出小红点展示决策，不在客户端计算未读数，也不对非当前选择助理展示 Tab 小红点。HarmonyOS 的 `UnReadManager.initUnReadState` 初始化时读取一次 `AgentTabNotify` 权限并缓存为 `agentTabNotifyEnabled`；当前代码中的权限读取、IM 监听、宿主 Tab 小红点设置和 H5 广播均保留 TODO，接入宿主能力后，后续流程只读取该缓存，不重复读取权限。命中权限后，从持久化缓存读取当前助理详情，按 `assistantAccount` 与 `bizRobotTag` 分流：严格匹配 `myAgent` 时请求员工助手服务端 GET `/getMessageData`，严格匹配 `uniassistant` 时跳过未读请求且不展示助理 Tab 小红点，其他类型请求 `/api/skill/sessions/unread`；切换助理通过 `onAssistantChanged` 清空旧的单例内存状态并刷新新助理。对外结果中的 `redDotVisible` 仅表示权限缓存，宿主助理 Tab 的实际展示由 SDK 结合助理类型、未读状态和 Tab 聚焦状态计算。直接调用未读查询、已读上报和会话查看态接口时只更新 SDK/宿主小红点状态，不广播 CUI；服务端 IM 未读通知、会话删除事件和网络恢复刷新才按各自规则触发广播。
...
1. SDK 冷启动时，`agentSkills` SDK 调用 `UnReadManager.initUnReadState`，读取一次 `AgentTabNotify` 权限并缓存结果；无权限则结束初始化，有权限才注册 IM 在线和离线通知，并从持久化缓存读取当前助理详情中的 `assistantAccount` 和 `bizRobotTag`。当前 HarmonyOS 版本的权限读取和宿主能力均为 TODO。若 `bizRobotTag=myAgent`，请求员工助手服务端 GET `/getMessageData` 获取全量未读消息，并将 `data.un_read_count > 0` 作为未读判断；若 `bizRobotTag=uniassistant`，跳过未读请求且不设置助理 Tab 小红点；其他情况请求 POST `/api/skill/sessions/unread`，调用 `applyServerResult` 重建当前助理会话缓存。最后按助理类型、权限、未读状态和助理 Tab 是否聚焦计算小红点，并通过宿主适配方法设置。
2. 切换助理时，容器层调用 `onAssistantChanged({ assistantDetail })`。HarmonyOS 先清空旧助理的 `partnerAccount`、`bizRobotTag`、查看态、会话缓存和已读序号；无权限、缺少账号或 `uniassistant` 时隐藏/跳过 Tab 小红点；其他情况按 `myAgent` 或 CUI 分支刷新，刷新前先用空状态触发一次 `onUnReadedChanged`，服务端成功后再覆盖缓存并刷新。
...
8. 网络从断开恢复为可用时，HarmonyOS 通过 `netAvailable` / `netLost` 识别“已知离线后恢复在线”，首次收到网络事件只建立基线，不刷新；恢复时若无权限、无当前助理、已有刷新请求或 `uniassistant`，直接跳过。其他情况保留旧缓存并按 `myAgent` 或 CUI 分支请求服务端，成功后覆盖缓存并调用 `onUnReadedChanged`，失败时保留旧缓存；`networkRefreshInFlight` 用于阻止并发恢复刷新。
    ...
    alt 无权限
        ...
        alt assistantAccount 有值
            alt bizRobotTag = myAgent
                ...
                SDK->>Memory: applyServerResult 写入会话未读缓存集合
...
sequenceDiagram
    ...
    alt assistantAccount 为空
        ...
        SDK->>Server: POST /api/skill/sessions/unread
        Server-->>SDK: 返回当前助理所有会话未读状态
        SDK->>Memory: applyServerResult 覆盖 sessionUnreadSet
        SDK->>SDK: onUnReadedChanged()
        SDK-->>Tab: 按 WeAgentCUI 小红点规则刷新助理 Tab
        SDK-->>SDK: HWH5INNER.eventListener 发送 agentskills_unreadChanged
    end
...
sequenceDiagram
    ...
    CUI->>SDK: getWeAgentUnreadMessage({ assistantAccount })
    SDK->>Server: POST /api/skill/sessions/unread
    Server-->>SDK: 返回当前助手未读状态
    SDK->>Memory: 写入内存缓存
    SDK-->>CUI: 返回当前助手未读状态
    CUI->>CUICache: 写入 CUI 未读缓存
    Hook->>Hook: 调用 getSessionMessageHistory 加载首屏历史消息
    alt 历史消息请求成功
        Hook->>Hook: 先写入历史 messages
    else 历史消息请求失败
        Hook->>Hook: 记录错误并保留空 messages
    end
    Hook->>Hook: 历史请求完成后注册 registerSessionListener
    Server-->>Hook: 后续通过 onMessage 下发实时消息
    Hook->>Hook: 将实时消息追加或更新到当前 messages
    alt onVisible 返回 visibility = 1
        ...
        SDK->>SDK: onUnReadedChanged(shouldBroadcast=false)
        SDK-->>Tab: 按 WeAgentCUI 小红点规则刷新助理 Tab
...
sequenceDiagram
    ...
    SDK->>Server: POST /api/skill/sessions/unread
    Server-->>SDK: 返回当前助手未读状态
    SDK->>Memory: applyServerResult 写入 sessionUnreadSet
...
sequenceDiagram
    ...
    SDK->>SDK: 读取当前 assistantAccount / bizRobotTag / 权限缓存
...
三端仅在网络状态由不可用变为可用时触发该流程；HarmonyOS 首次收到网络事件只建立状态基线，不立即刷新。Android 使用
...
1. SDK 初始化时调用 `UnReadManager.initUnReadState`；该方法先读取一次 `AgentTabNotify` 权限并写入 `agentTabNotifyEnabled` 内存缓存。HarmonyOS 当前 `readAgentTabNotifyEnabled`、IM 监听注册、宿主 Tab 小红点设置和 H5 广播均为 TODO；无权限直接结束，有权限才继续初始化，后续流程直接读取该缓存，不重复读取权限。
2. `initUnReadState` 从持久化缓存读取当前助理详情中的 `assistantAccount` 和 `bizRobotTag`；若 `assistantAccount` 不存在，则跳过服务端未读查询。
3. 冷启动时若 `bizRobotTag=myAgent`，SDK 请求员工助手服务端 GET `/getMessageData` 获取全量未读消息，根据 `data.un_read_count > 0` 判断是否有未读；若 `bizRobotTag=uniassistant`，跳过未读请求且不设置助理 Tab 小红点；其他情况请求 POST `/api/skill/sessions/unread` 拉取该助理所有会话未读状态。`HttpClient` 统一处理服务端响应包装，允许成功码 `0` 或 `200`，失败时抛出服务端错误。
4. SDK 新增 `UnReadManager`，统一收口未读查询、已读上报、会话查看态标记、助理切换、内存缓存和 H5 内部广播处理逻辑。
5. SDK 使用 `applyServerResult` 根据服务端 POST `/api/skill/sessions/unread` 返回的 `unreadSessionList` 整体重建当前助理会话未读内存缓存集合；如果列表中的会话 ID 与当前查看会话一致，则该会话设置为已读，其他会话设置为未读，并保存服务端返回的 `maxSeq`。
6. SDK 使用 `onUnReadedChanged`，按 `bizRobotTag`、`agentTabNotifyEnabled`、助理 Tab 聚焦状态和未读状态计算助理 Tab 小红点；实际设置 / 隐藏助理 Tab 小红点和向 CUI 广播均为宿主适配 TODO。`shouldBroadcast=false` 的直接 SDK 查询、已读上报和查看态操作只刷新宿主小红点，不广播 CUI。
...
8. SDK 提供获取助理未读消息接口，入参为 `assistantAccount` 和可选 `sessionIds`，入口先检查 `isAgentTabNotifyEnabled`；无权限时抛出 `4011`，不校验参数、不请求服务端、不读取或修改未读状态。权限通过后请求 POST `/api/skill/sessions/unread` 并写入内存缓存。
9. SDK 提供已读上报能力，入口先检查权限；`UnReadManager` 按会话记录已上报的最大 `readSeq`，重复或更小的序号直接成功返回。成功后更新本地会话为已读并刷新宿主小红点，但不广播 CUI。
10. SDK 提供当前会话查看态接口，入口先检查权限；`onSessionViewing` 记录当前查看会话、将其设为已读并刷新宿主小红点，但不广播 CUI；停留期间忽略该会话的 `session.unread` 推送。`onSessionViewingEnd` 仅在入参等于当前查看会话时清除查看标记，不主动刷新小红点。
11. SDK 预留面向 `weAgentCUI` 的未读状态 H5 内部广播；当前 HarmonyOS `broadcastUnreadChanged` 为 TODO。服务端 CUI IM 推送、会话删除和网络恢复刷新使用默认广播路径，直接 SDK 接口和员工助手通知使用 `shouldBroadcast=false`。
...
14. IM 在线和离线通知由宿主透传到 `handleWeAgentUnreadImNotifyBroadcast`，按 `notifyModuleId` 分流：`uni-assistant` 进入员工助手处理，`welink-athena` 进入 CUI 处理，其他模块忽略。员工助手通知读取 `notify_content.un_read_count`；CUI 通知读取 `notify_type` 和当前代码使用的 `notyfy_content`，只处理当前 `assistantAccount` 的 `session.unread` / `session.read`，并按 `welinkSessionId` 刷新缓存。员工助手通知不广播 CUI，CUI 通知默认广播。
15. SDK 新增网络状态恢复监听；HarmonyOS 使用系统 `netAvailable` / `netLost`，仅在已知离线后恢复在线时触发 `onNetworkReconnected`。补偿请求前不清空未读内存缓存；查询成功后复用 `applyServerResult` 和 `onUnReadedChanged` 刷新助理 Tab，并按默认规则广播 CUI；查询失败或助理已切换时保留原缓存。`networkRefreshInFlight` 防止恢复期间并发刷新。
...
| 能力 | 用途 | 本方案职责 | TODO |
|---|---|---|---|
| 获取 ABTest 权限接口 | 判断 `AgentTabNotify` 是否命中 | `initUnReadState` 调用 `readAgentTabNotifyEnabled` 并缓存结果；`SkillSDK` 公共未读接口通过 `isAgentTabNotifyEnabled` 拦截 | 当前 HarmonyOS 方法为 TODO，默认返回 `false`；二次开发时接入实际 ABTest 方法 |
| 监听 IM 在线 / 离线通知接口 | 接收员工助手未读和 CUI `session.unread` / `session.read` 推送 | `handleWeAgentUnreadImNotifyBroadcast` 按 `notifyModuleId` 分流后交给 `UnReadManager` | 当前 `registerImUnreadNotifications` 为 TODO；二次开发时接入实际 IM 通知注册 / 反注册方法 |
| 设置助理 Tab 小红点接口 | 展示或隐藏宿主助理 Tab 小红点 | `onUnReadedChanged` 计算 `showHostRedDot` 后调用 `setHostWeAgentTabRedDot` | 当前 `setHostWeAgentTabRedDot` 为 TODO；二次开发时接入宿主 UI 方法和 Tab 聚焦状态读取 |
| CUI 未读广播接口 | 将 SDK 未读状态同步给 `weAgentCUI` | `onUnReadedChanged` 在 `shouldBroadcast=true` 时调用 `broadcastUnreadChanged` | 当前 `broadcastUnreadChanged` 为 TODO；二次开发时接入 `HWH5INNER.eventListener` |
...
2. `initUnReadState` 负责 SDK 冷启动未读初始化：先读取一次 `AgentTabNotify` 权限并写入 `agentTabNotifyEnabled` 内存缓存，再注册 IM 在线/离线通知，从持久化缓存读取当前助理详情中的 `assistantAccount` 和 `bizRobotTag`，并按 `bizRobotTag` 分流冷启动未读拉取逻辑。HarmonyOS 当前三类宿主能力均为 TODO。
...
4. `applyServerResult` 负责处理初始化、主动查询和网络恢复返回的 `/api/skill/sessions/unread` 结果：按 `unreadSessionList[].sessionId` 重建会话未读缓存集合，当前查看会话设置为 `hasUnRead=false`，其他会话设置为 `hasUnRead=true`，并保存 `maxSeq`；主动查询传入 `sessionIds` 时，对服务端未返回的会话补写已读状态。
5. `onUnReadedChanged` 负责读取 `agentTabNotifyEnabled`、`bizRobotTag`、助理 Tab 聚焦状态和未读缓存，按统一规则产出实际展示决策；广播结果中的 `redDotVisible` 仅透传权限缓存。`shouldBroadcast=false` 时只调用宿主 Tab 小红点设置，不调用 CUI 广播。
...
9. `onNetworkReconnected` 负责网络恢复后的未读状态补偿查询；该方法读取当前内存中的 `partnerAccount`、`bizRobotTag` 和权限缓存，按冷启动相同的 `bizRobotTag` 分支查询未读状态，并复用缓存覆盖、Tab 刷新和 CUI 广播流程。

#### 4.2.2 冷启动当前助理类型判断

`initUnReadState` 只在 `agentTabNotifyEnabled=true` 后继续执行冷启动未读初始化。SDK 从持久化缓存读取当前助理详情，至少使用 `assistantAccount` 和 `bizRobotTag` 两个字段：

1. `assistantAccount` 为空时，不请求服务端未读接口，不设置助理 Tab 小红点。
2. `bizRobotTag=myAgent` 时，请求员工助手服务端 `GET /getMessageData` 获取全量未读消息；`data.un_read_count > 0` 后，若助理 Tab 不聚焦则设置助理 Tab 小红点，否则不设置助理 Tab 小红点。
3. `bizRobotTag=uniassistant` 时，表示员工助手分身，不请求 WeAgentCUI 未读接口，不设置助理 Tab 小红点。
4. 其他 `bizRobotTag` 按 WeAgentCUI 页面处理，请求 POST `/api/skill/sessions/unread`，按 `unreadSessionList` 更新 `UnReadManager` 会话未读缓存集合，并通过 `onUnReadedChanged` 根据助理 Tab 聚焦状态和集合中的 `hasUnRead` 刷新助理 Tab；默认广播路径再通知 `weAgentCUI`。
...
2. 切换助理时，`onAssistantChanged({ assistantDetail })` 先清空旧助理的 `sessionUnreadSet`、查看态和已读序号，再按新助理详情中的 `assistantAccount`、`bizRobotTag` 拉取未读状态，并用服务端返回结果覆盖整个未读消息列表。
3. `sessionUnreadSet` 是当前助理的会话未读缓存 Map，按会话 ID 去重；key 为 `welinkSessionId`，value 为 `{ welinkSessionId, hasUnRead: boolean, maxSeq: number }`。
4. `applyServerResult` 根据服务端 `/api/skill/sessions/unread` 返回的 `unreadSessionList` 整体重建或覆盖 `sessionUnreadSet`：若 `unreadSessionList[].sessionId` 与当前查看会话一致，则该会话写为 `{ hasUnRead: false, maxSeq }`；其他会话写为 `{ hasUnRead: true, maxSeq }`。主动查询传入 `sessionIds` 时，服务端未返回的会话会补写为已读状态。
5. `assistantUnread` 不单独计算未读数，只作为从 `sessionUnreadSet` 派生出的布尔值：集合中存在任一 `hasUnRead=true` 时为 `true`，否则为 `false`。
6. 助理 Tab 小红点根据当前单例缓存中的 `bizRobotTag`、`myAgentUnread`、`sessionUnreadSet`、权限和宿主 Tab 聚焦状态判断；不能简化为“存在 `hasUnRead=true` 就展示”。
...
10. SDK 初始化查询会写入当前助理的单例内存缓存；员工助手通知只在当前 `bizRobotTag=myAgent` 时处理，CUI 通知只处理当前 `assistantAccount` 的通知，非当前助理通知直接忽略，不写入缓存。
...
2. `SkillSDK` 入口先读取 `UnReadManager.isAgentTabNotifyEnabled()`；权限为 `false` 时抛出错误码 `4011`，错误信息为 `Permission denied: AgentTabNotify is disabled`，不执行参数校验、网络请求或缓存操作。
3. SDK 收到服务端结果后，调用 `applyServerResult` 将 `data.unreadSessionList` 写入 `sessionUnreadSet`，再根据集合中是否存在 `hasUnRead=true` 生成 `assistantUnread`。
4. SDK 将 `data.unreadSessionList[].sessionId` 映射为会话缓存 key 和 `sessions[].welinkSessionId`，将 `data.unreadSessionList[].maxSeq` 映射为 `maxSeq`；若会话 ID 与当前查看会话一致，则设置 `hasUnRead=false`，其他会话设置 `hasUnRead=true`。
5. 如果入参传了 `sessionIds`，不在 `unreadSessionList` 内的会话补齐为 `hasUnRead=false`；如果未传 `sessionIds`，只返回服务端列表处理后的会话状态。
6. 成功查询后调用 `onUnReadedChanged('server', false)`，刷新宿主 Tab 小红点但不广播 CUI。
7. SDK 将缓存的 `AgentTabNotify` 权限映射为 `redDotVisible`；宿主助理 Tab 的实际展示由 SDK 独立计算。
8. 请求失败且当前单例缓存的 `partnerAccount` 与入参一致时，返回 `source=cache` 的当前缓存；缓存属于其他助理时透传错误，不构造空结果。
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `code` | `number` | `0` 或 `200` 表示请求成功；其他值按请求失败处理 |
| `data.un_read_count` | `number` | 员工助手全量未读消息数，SDK 只消费是否大于 `0`，不展示具体数字 |

处理规则：

1. SDK 仅在当前助理 `bizRobotTag=myAgent` 且权限开启时调用 `GET /getMessageData`。
2. `HttpClient` 统一接受服务端成功码 `0` 或 `200`；成功后 `data.un_read_count > 0` 表示员工助手存在未读，字段缺失或非正数时按无未读处理。
3. 员工助手 Tab 小红点仍受助理 Tab 聚焦状态控制：Tab 不聚焦且存在未读时展示，Tab 聚焦时不展示。
4. SDK 不把 `un_read_count` 写入 `sessionUnreadSet`，只更新当前 `myAgent` 的 `myAgentUnread`；`redDotVisible` 始终仅表示权限缓存。
5. 请求失败、超时、服务端失败码或响应 JSON 解析失败时，`refreshCurrentAssistantUnread` 返回失败并保留原内存状态；初始化/切换场景不会构造额外空缓存，等待后续 IM 通知或下一次补偿查询校正。
...
1. `SkillSDK` 入口先检查 `UnReadManager.isAgentTabNotifyEnabled()`；权限关闭时抛出 `4011` 和 `Permission denied: AgentTabNotify is disabled`。
2. `UnReadManager` 按 `welinkSessionId` 记录已上报的最大 `readSeq`；小于或等于已记录值时直接返回成功，不请求服务端。
3. 服务端上报成功后，SDK 更新对应会话为已读，调用 `onUnReadedChanged('readReport', false)` 刷新宿主 Tab 小红点，但不广播 CUI；后续服务端 IM `session.read` 通知仍可再次校正缓存并广播。
...
1. `SkillSDK` 入口先检查 `UnReadManager.isAgentTabNotifyEnabled()`；权限关闭时抛出 `4011` 和 `Permission denied: AgentTabNotify is disabled`。
2. `weAgentCUI` 在 `onVisible` 返回 `visibility=1` 或页面内切换会话后调用 `onSessionViewing({ welinkSessionId })`，SDK 将该会话记录为当前查看会话，并将内存缓存中的该会话未读态设置为已读。
3. 当前查看会话停留期间，SDK 收到服务端 `session.unread` 推送时，若推送会话等于当前查看会话，忽略该会话的未读态更新；其他会话仍按正常推送处理。查看态调用刷新宿主 Tab 小红点，但不广播 CUI。
4. `weAgentCUI` 在 `onVisible` 返回 `visibility=0`、离开会话页面或页面销毁时调用 `onSessionViewingEnd({ welinkSessionId })`；SDK 仅在入参与当前查看会话一致时清除查看标记，不主动刷新小红点。
5. 清除查看标记后，该会话恢复服务端正常推送处理；后续服务端推送的未读状态可以重新点亮会话 item 小红点。
...
1. `getWeAgentUnreadMessage` 测试：
   - 权限校验：`agentTabNotifyEnabled=false` 时由 `SkillSDK` 直接返回 `4011` 和 `Permission denied: AgentTabNotify is disabled`，不执行参数校验和网络请求。
   - 入参校验：权限开启后，`assistantAccount` 必填，`sessionIds` 可选；`assistantAccount` 为空时不请求服务端。
   - 服务端请求校验：新版本 SDK 调用 POST `/api/skill/sessions/unread`，请求体包含 `assistantAccount`，传入会话列表时包含 `sessionIds`。
   ...
   - 降级校验：服务端失败时，仅当单例缓存属于当前助理时返回 `source=cache`；缓存缺失或属于旧助理时透传错误，不构造空结果。
...
2. `reportWeAgentSessionRead` 测试：
   - 权限校验：`agentTabNotifyEnabled=false` 时返回 `4011`，不执行参数校验、已读序号去重或服务端请求。
   ...
   - 广播校验：上报成功只刷新宿主 Tab 小红点，不发送 `agentskills_unreadChanged`。
...
3. `onSessionViewing` 测试：
   - 权限校验：`agentTabNotifyEnabled=false` 时返回 `4011`，不更新查看态。
...
5. 员工助手 `GET /getMessageData` 测试：
   ...
   - 空值校验：`un_read_count=0`、字段缺失或非数字时按无未读处理；HTTP 或服务端失败码按刷新失败处理并保留原内存状态。
   - 聚焦态校验：`un_read_count > 0` 且助理 Tab 聚焦时不展示小红点，非聚焦时展示小红点。

#### 4.2.9 IM 在线和离线通知处理

宿主调用 `SkillSDK.handleWeAgentUnreadImNotifyBroadcast(payload)` 透传 IM 通知，HarmonyOS 只读取以下两个外层字段：

| 字段 | 值 | 处理方法 |
|---|---|---|
| `notifyModuleId` | `uni-assistant` | `handleEmployeeAssistantImUnreadNotifyData` |
| `notifyModuleId` | `welink-athena` | `handleCuiImUnreadNotifyData` |

其他模块直接忽略；`notifyData` 无法解析为对象时也直接忽略。员工助手通知只在当前 `bizRobotTag=myAgent` 时消费：

1. 从 `notifyData.notify_content` 读取 `un_read_count`；字符串 JSON 由类型转换器解析为对象。
2. `un_read_count > 0` 表示员工助手有未读，更新 `myAgentUnread`；`un_read_count <= 0`、缺失或无法转换时按无未读处理。
3. 更新后调用 `onUnReadedChanged('serverPush', false)`，刷新宿主 Tab 小红点，不广播 CUI。

CUI 服务端推送的未读 payload（外层宿主字段）：

```json
{
  "notifyModuleId": "welink-athena",
  "notifyData": {
...
1. 仅当 `notifyModuleId=welink-athena`、`notify_type` 为 `session.unread` 或 `session.read`，且 `notyfy_content.assistantAccount` 等于当前 `partnerAccount` 时处理；非当前助理通知直接忽略。
2. `session.unread` 时，按 `welinkSessionId` 刷新对应 `sessionUnreadSet`，写入 `{ hasUnRead: true, maxSeq }`；如果该会话正处于查看态，则忽略本次未读通知。
3. `session.read` 时，按 `welinkSessionId` 写入 `{ hasUnRead: false, maxSeq }`；`readSeq` 不参与小红点计算。
4. 更新缓存后调用 `onUnReadedChanged('serverPush')`，刷新宿主 Tab 小红点并按默认参数广播 CUI。
...
1. `initUnReadState` 冷启动按 `bizRobotTag` 分支处理：`myAgent` 根据员工助手 GET `/getMessageData` 返回的 `data.un_read_count` 更新 `myAgentUnread`，`uniassistant` 不请求且不设置助理 Tab 小红点，其他 WeAgentCUI 场景通过 `applyServerResult` 更新 `sessionUnreadSet` 后调用 `onUnReadedChanged`。
2. `onUnReadedChanged` 按 `agentTabNotifyEnabled`、`bizRobotTag`、宿主 Tab 聚焦状态、`myAgentUnread` 或 `sessionUnreadSet` 计算 `showHostRedDot`；`redDotVisible` 只反映权限缓存。是否设置宿主小红点和是否广播 CUI 分别由 `setHostWeAgentTabRedDot`、`broadcastUnreadChanged` 负责，当前均为 TODO。
3. 冷启动无权限时不注册 IM / 网络监听、不请求未读接口、不调用小红点设置和 CUI 广播；SkillSDK 的四个公开未读接口也在入口返回 `4011`。
4. 切换助理后由容器层触发 `onAssistantChanged({ assistantDetail })`；SDK 清空旧状态，先按新助理状态调用一次 `onUnReadedChanged('assistantChanged')`，再按 `assistantAccount` 和 `bizRobotTag` 刷新，成功后再次覆盖缓存并刷新。
5. `getWeAgentUnreadMessage`、`reportWeAgentSessionRead`、`onSessionViewing` 的状态变更均使用 `shouldBroadcast=false`，因此只刷新宿主 Tab，不发送 `agentskills_unreadChanged`；`onSessionViewingEnd` 只清除查看标记。
6. `weAgentCUI` 收到 `agentskills_unreadChanged` 后只处理当前助理数据：根据 `redDotVisible` 与 `sessions[].hasUnRead` 更新 CUI 未读缓存并刷新历史会话入口和 item 小红点；当前 HarmonyOS SDK 尚未接入该广播实现。
7. WebSocket 收到 `session.deleted` 时，`SkillSDK` 调用 `onSessionDeleted` 删除对应会话的未读状态和已读序号；如果删除的是当前查看会话则同时清除查看态，状态发生变化时按默认广播规则刷新小红点。

#### 4.2.11 助理 Tab 展示规则

1. 助理 Tab 只消费当前选择助理的 `bizRobotTag`、`AgentTabNotify` 权限状态、助理 Tab 聚焦状态、员工助手未读态或 WeAgentCUI 会话未读缓存集合。
2. `bizRobotTag=myAgent` 时，只有权限开启、`myAgentUnread=true` 且助理 Tab 不聚焦时展示小红点；助理 Tab 聚焦时不展示小红点。
3. `bizRobotTag=uniassistant` 时，无论是否有未读，助理 Tab 都不展示小红点。
4. 其他 `bizRobotTag` 时，必须先命中权限；若助理 Tab 不聚焦，只要当前助理会话存在任一 `hasUnRead=true` 的会话就展示小红点；若助理 Tab 聚焦，则排除当前查看会话后，其他会话存在 `hasUnRead=true` 时才展示小红点。
5. 不读取、不展示、不计算未读数。
...
1. 页面初始化后调用 `HWH5EXT.registerEventListener` 注册 `agentskills_unreadChanged`，并调用 `HWH5.addEventListener({ type: 'onVisible', func })` 监听可见性；这两个页面级监听不受历史消息请求成败影响。
2. 页面初始化阶段可以调用 `getWeAgentUnreadMessage({ assistantAccount })` 获取当前助理未读状态，并将接口返回写入 CUI 未读缓存；SDK 同步维护 `UnReadManager` 内存缓存，但页面不得因为初始化代码执行直接上报已读。HarmonyOS 和 iOS 冷启动预加载会执行页面代码，但不代表用户已前台打开 `weAgentCUI`。
3. `useChatSession` 先调用 `getSessionMessageHistory` 加载当前会话首屏历史消息。历史请求成功时先将历史消息写入 `messages`；请求失败时记录错误并保留可渲染的空消息状态；无论成功还是失败，在 `loadMessages` 完成后才注册当前会话的 `registerSessionListener`。
4. 历史消息请求完成前不得注册当前会话的实时监听，避免实时消息先写入渲染缓存后又被历史消息初始化覆盖。历史请求成功后，实时消息追加在首屏历史消息之后；历史请求失败后，实时消息仍正常渲染。会话已切换、请求已取消或请求 epoch 已失效时，不注册旧会话监听。
5. 回调中 `visibility = 1` 表示页面在移动端前台显示，页面先调用 `onSessionViewing({ welinkSessionId })` 标记当前查看会话，并在已渲染消息后调用 `reportWeAgentSessionRead({ welinkSessionId, readSeq })`；上报成功后 SDK 更新内存缓存与助理 Tab 小红点，不广播 `weAgentCUI` 未读状态。
6. 回调中 `visibility = 0` 表示页面不可见，页面调用 `onSessionViewingEnd({ welinkSessionId })` 清除当前查看会话标记，不触发已读上报，不主动隐藏小红点。
7. Android 冷启动不会预加载 `weAgentCUI`，但仍按同一规则处理：页面初始化注册广播和可见性监听，收到 `visibility = 1` 后再标记会话查看态、上报已读和刷新未读状态。
8. 页面需要维护本次前台可见周期的幂等标记，避免同一 `welinkSessionId` 在连续 `visibility = 1` 回调中重复上报已读；当会话或助理切换后重置该标记。
9. 如果存在非当前会话的未读消息，则历史会话列表按钮图标展示小红点。
10. 打开历史会话列表后，根据 SDK 映射后的 `sessions[].hasUnRead` 给对应会话 item 展示小红点。
11. 当前正在打开的会话不展示会话 item 小红点；当前会话被标记已读后，其他会话未读状态保持不变。
12. `agentskills_unreadChanged` 的 `partnerAccount` 与当前页面助理一致时，按广播 payload 更新 CUI 未读缓存，并立即刷新历史会话入口和已渲染会话 item 的小红点；不一致时只忽略 UI 刷新，不主动请求服务端，也不修改当前页面小红点。
13. 页面 CUI 未读缓存来源包括 `getWeAgentUnreadMessage` 返回结果和 `agentskills_unreadChanged` 事件；用户从其他页面切回助理 Tab 时，不再调用 `getWeAgentUnreadMessage`，直接从该缓存读取当前会话未读状态和 `maxSeq` 作为 `readSeq` 上报。
...
16. `ai-chat-viewer` 通过 `useChatSession` 的 `registerSessionListener.onMessage` 接收服务端实时消息，并将消息归一后更新页面 `messages`；该监听仅在当前会话的历史消息请求完成后注册。`weAgentCUI` 监听当前会话 `messages` 变化，识别新增的 AI 回复消息。
...
20. 页面 `onVisible` 返回 `visibility=0`、离开会话或销毁时调用 `onSessionViewingEnd({ welinkSessionId })`，通知 SDK 清除当前查看会话标记并恢复该会话正常推送处理。
21. 前端小红点展示位置固定为三处：CUI 底部操作区的历史会话入口、历史侧边栏中的会话 item，以及宿主助理 Tab；创建会话按钮、输入框、发送按钮和历史侧边栏关闭按钮不展示未读小红点。
22. 历史入口使用“除当前会话外存在 `hasUnRead=true` 的会话”作为展示条件，会话 item 使用对应 `sessions[].hasUnRead=true` 作为展示条件；两者都必须同时满足 `redDotVisible=true`。当前会话不展示会话 item 小红点。
23. 前端小红点不展示数字，统一为直径 `8px` 的圆形，颜色为 `rgba(253, 103, 32, 1)`；历史入口按钮使用相对定位，圆点位于右上角 `top: 0; right: 0`，会话 item 圆点与标题之间保留 `8px` 间距。
24. 历史入口按钮尺寸为 `32px × 32px`、白色背景、圆角 `20px`；会话 item 高度为 `40px`。PC 和移动端复用相同的圆点尺寸、颜色和展示条件，侧边栏抽屉或常驻布局不改变未读语义。

#### 4.2.13 HarmonyOS 当前实现基线

以下内容以 HarmonyOS 当前 `UnReadManager.ets`、`SkillSDK.ets` 和 `WebSocketManager.ets` 的实现为准；与其他端的抽象方案存在差异时，先以此实现行为为准：

1. 初始化由 `SkillSDK` 构造流程异步调用 `initUnReadState`。`UnReadManager` 读取一次权限并保存到 `agentTabNotifyEnabled`；权限关闭时直接结束初始化，不注册 IM / 网络监听，也不发起未读查询。当前 `readAgentTabNotifyEnabled` 默认返回 `false`，等待宿主 ABTest 能力接入。
2. 权限开启后注册 IM 监听和 HarmonyOS `netAvailable` / `netLost` 监听，从 `WeAgentStore.loadCurrentDetail()` 读取当前助理的 `partnerAccount` 与 `bizRobotTag`。账号为空时跳过刷新。
3. `refreshCurrentAssistantUnread` 严格按字符串值分流：`myAgent` 调用 `GET /getMessageData`，`uniassistant` 直接跳过，其他值调用 `POST /api/skill/sessions/unread`。请求成功后分别更新 `myAgentUnread` 或通过 `applyServerResult` 重建 `sessions`；请求失败只记录日志并返回失败状态。
4. `/getMessageData` 通过 `skipBaseUrl=true` 请求，不拼接 SDK 的普通或助理 BaseUrl；CUI 未读接口请求体使用 `{ assistantAccount: partnerAccount, sessionIds? }`。统一 `HttpClient` 将服务端 `code` 转为 number，`0` 和 `200` 视为成功，其他值抛出服务端错误。
5. `getWeAgentUnreadMessage` 成功后调用 `onUnReadedChanged('server', false)`；失败时只有当前内存 `partnerAccount` 与入参一致才返回内存快照，否则抛出错误。`reportWeAgentSessionRead` 成功后调用 `onUnReadedChanged('readReport', false)`；`onSessionViewing` 成功后调用 `onUnReadedChanged('sessionViewing', false)`；这三个接口均不广播 CUI，`onSessionViewingEnd` 仅清除匹配的查看态。
6. `handleWeAgentUnreadImNotifyBroadcast` 只读取外层 `notifyModuleId` 和 `notifyData`。`uni-assistant` 使用 `notify_content.un_read_count` 更新 `myAgentUnread`，且不广播 CUI；`welink-athena` 使用 `notify_type` 和代码当前读取的 `notyfy_content`，按 `assistantAccount` 校验当前助理后更新会话缓存并按默认规则广播。
7. `session.unread` 对当前查看会话直接忽略，其他会话写入 `hasUnRead=true`；`session.read` 写入 `hasUnRead=false`。WebSocket `session.deleted` 不经过 IM 通知入口，由 `SkillSDK` 直接调用 `onSessionDeleted` 删除对应会话缓存和已读序号；若删除的是查看会话，同时清除查看态。
8. `shouldShowHostWeAgentTabRedDot` 的实际判断为：无权限或 `uniassistant` 永不展示；`myAgent` 只看 `myAgentUnread` 且要求 Tab 未聚焦；其他助理在 Tab 未聚焦时任一会话未读即可展示，Tab 聚焦时排除当前查看会话后再判断。`setHostWeAgentTabRedDot` 和 `broadcastUnreadChanged` 当前均为 TODO。
9. 网络监听第一次收到 `netAvailable` 或 `netLost` 只建立状态基线；仅在已知离线后收到 `netAvailable` 时触发补偿查询。补偿查询不清空旧缓存，使用 `networkRefreshInFlight` 防并发；`uniassistant`、无账号或无权限时跳过。

### 4.3 兼容与边界

1. 未命中 `AgentTabNotify` 权限时，初始化不继续未读能力；四个公开未读接口由 `SkillSDK` 返回 `4011` 和 `Permission denied: AgentTabNotify is disabled`，端侧默认不展示小红点。
...
4. 初始化或切换助理的刷新请求失败时，HarmonyOS 保留刷新前的清空结果，不构造空的服务端回退结果；网络恢复刷新失败时保留旧缓存。独立调用 `getWeAgentUnreadMessage` 失败时，仅当单例缓存账号与入参一致才返回内存缓存，否则透传错误。
...
1. SDK 初始化入口：调用 `UnReadManager.initUnReadState`，读取并缓存 `AgentTabNotify` 权限；有权限后注册 IM / 网络监听，再从持久化缓存读取当前助理详情中的 `assistantAccount` 和 `bizRobotTag`。`bizRobotTag=myAgent` 时请求员工助手服务端 GET `/getMessageData` 并按 `data.un_read_count`、权限和助理 Tab 聚焦状态判断小红点；`bizRobotTag=uniassistant` 时不请求且不设置助理 Tab 小红点；其他情况请求 POST `/api/skill/sessions/unread` 并按 CUI 规则判断展示。HarmonyOS 当前这些宿主能力为 TODO。
2. `UnReadManager`：SDK 内部未读管理模块统一管理 `initUnReadState`、`onAssistantChanged`、`applyServerResult`、`onUnReadedChanged`、未读查询、已读上报、会话查看态、内存缓存和广播。
3. `getWeAgentUnreadMessage`：新增 SDK 接口，入参为 `assistantAccount` 和可选 `sessionIds`，请求服务端 POST `/api/skill/sessions/unread`，返回该助理会话未读状态；请求失败时可降级读取内存缓存。
...
1. `SkillClientSdkInterfaceV2.md`：补充 SDK 初始化权限判断、持久化缓存读取当前助理详情、`bizRobotTag` 冷启动分流、员工助手服务端 GET `/getMessageData` 与 `data.un_read_count` 判断、助理 Tab 聚焦状态小红点规则、CUI `session.unread/session.read` IM payload、`onAssistantChanged({ assistantDetail })` 入参、宿主能力接入 TODO、`UnReadManager`、`initUnReadState`、`applyServerResult`、`onUnReadedChanged`、四个未读接口定义、服务端协议和 `agentskills_unreadChanged` H5 内部广播协议。
...
1. SDK 初始化在存在当前助理账号时按 `bizRobotTag` 选择冷启动查询路径：`myAgent` 请求员工助手 GET `/getMessageData` 并只消费 `data.un_read_count`，其他 WeAgentCUI 场景请求当前助理会话未读接口；小红点展示再结合权限和助理 Tab 聚焦状态判断，两类请求都不返回消息内容。
2. `getWeAgentUnreadMessage` 按 `assistantAccount` 和可选 `sessionIds` 请求服务端未读接口，服务端返回 `unreadSessionCount`、`unreadSessionList[].sessionId` 和 `unreadSessionList[].maxSeq`，不返回消息内容。
...
| `applyServerResult` 执行 | `[WeAgentUnread] apply_unread_status` | `assistantAccount`、`currentViewingSessionId`、`sessionCount`、`unreadCount` |
...
4. 当前助理 `bizRobotTag=myAgent`、权限开启、员工助手服务端 GET `/getMessageData` 返回成功且 `data.un_read_count > 0`，并且助理 Tab 不聚焦时，校验助理 Tab 展示小红点。
5. 当前助理 `bizRobotTag=myAgent`、权限开启、员工助手服务端 GET `/getMessageData` 返回成功且 `data.un_read_count > 0`，但助理 Tab 聚焦时，校验助理 Tab 不展示小红点。
6. 当前助理 `bizRobotTag=myAgent` 且员工助手服务端 GET `/getMessageData` 返回成功、`data.un_read_count = 0` 时，校验助理 Tab 不展示小红点。
7. 当前助理 `bizRobotTag=uniassistant` 时，校验不请求 WeAgentCUI 未读接口，不展示助理 Tab 小红点。
8. 当前助理为其他 `bizRobotTag` 且助理 Tab 不聚焦时，校验 SDK 请求 POST `/api/skill/sessions/unread` 拉取当前助理所有会话未读状态，调用 `applyServerResult` 将 `unreadSessionList` 写入 `sessionUnreadSet`；权限开启且集合中存在 `hasUnRead=true` 时展示助理 Tab 小红点。
...
18. 网络从断开恢复为可用时，当前助理 `bizRobotTag=myAgent`，校验 SDK 调用员工助手服务端 GET `/getMessageData`，根据 `data.un_read_count` 和助理 Tab 聚焦状态刷新助理 Tab 小红点，并输出网络恢复成功日志。
19. 网络从断开恢复为可用时，当前助理 `bizRobotTag=uniassistant`，校验 SDK 不请求 WeAgentCUI 未读接口，不展示助理 Tab 小红点。
20. 网络从断开恢复为可用时，当前助理为其他 `bizRobotTag`，校验 SDK 请求 POST `/api/skill/sessions/unread`，调用 `applyServerResult` 覆盖单例未读缓存，再调用 `onUnReadedChanged` 刷新助理 Tab并按默认规则广播给 `weAgentCUI`。
...
25. CUI 页面初始化后调用 `getWeAgentUnreadMessage({ assistantAccount })` 获取当前会话所有未读消息并写入 CUI 未读缓存，存在非当前会话未读时历史会话按钮展示小红点。
25.1. `useChatSession` 首次调用 `getSessionMessageHistory` 成功时，校验历史消息先进入 `messages`，之后才注册 `registerSessionListener`；服务端实时消息出现在历史消息之后，不被历史初始化覆盖。
25.2. `getSessionMessageHistory` 失败时，校验错误不阻断 `registerSessionListener` 注册，后续实时消息仍能渲染；会话切换或请求失效时，校验旧会话监听不会被注册。
26. 用户从其他页面切回助理 Tab，`onVisible` 返回 `visibility=1` 时，校验 CUI 不再调用 `getWeAgentUnreadMessage`，而是先调用 `onSessionViewing({ welinkSessionId })`，再从 CUI 未读缓存读取当前会话未读状态和 `maxSeq`，并调用 `reportWeAgentSessionRead({ welinkSessionId, readSeq: maxSeq })` 上报已读，SDK 更新内存缓存与助理 Tab 小红点，不广播 CUI 未读状态。
27. `weAgentCUI` 页面 `visibility=0` 时，校验页面调用 `onSessionViewingEnd({ welinkSessionId })` 清除当前查看会话标记，且不触发已读上报。
28. 打开历史会话列表后，SDK 调用 `getWeAgentUnreadMessage({ assistantAccount, sessionIds })`，校验服务端 `unreadSessionList` 内与当前 WeAgentCUI 会话一致的会话被映射为 `hasUnRead=false`，其他会话被映射为 `hasUnRead=true`，不在列表内的会话被补齐为 `hasUnRead=false`；有未读的会话 item 展示小红点，无未读的 item 不展示。
...
47. 独立调用 `getWeAgentUnreadMessage` 请求失败且当前进程已有内存缓存、缓存 `partnerAccount` 与入参一致时，返回 `source=cache` 的内存状态。
48. 初始化或切换助理请求失败时，校验刷新前缓存已被清空，不构造服务端空结果；网络恢复请求失败时，校验旧缓存保持不变并等待后续刷新或 IM 增量更新。
...
2. Mock 权限读取返回 `true`，当前助理 `bizRobotTag=myAgent` 且员工助手 GET `/getMessageData` 返回成功、`data.un_read_count > 0`，助理 Tab 不聚焦时校验展示助理 Tab 小红点；助理 Tab 聚焦时校验不展示。
...
建议采用“服务端管控 + SDK 当前助理单例内存缓存 + IM 通知增量更新 + 宿主 Tab 展示决策”的方案。HarmonyOS 当前严格按 `myAgent`、`uniassistant` 和其他类型分流：员工助手只消费 `un_read_count`，`uniassistant` 跳过未读处理，其他类型维护 `sessionUnreadSet`；`getWeAgentUnreadMessage`、`reportWeAgentSessionRead` 和查看态接口由 `SkillSDK` 统一做权限拦截，直接调用不广播 CUI。CUI IM 通知、WebSocket `session.deleted` 和网络恢复查询按默认广播路径刷新状态。权限读取、IM 监听、宿主 Tab 小红点设置和 CUI 广播仍需二次开发接入。
